...
class DeliveryExecutive :
    ...
    def get_orders_details(pysql, deliveryexecutive_id, flag = 0) :
        if flag == 0:
            sql_stmt =  """ WITH T1 AS (  \
                            	SELECT \
					Order_ID, Address_ID, Payment_Method, Total_Price, Status \
                           	 FROM \
					Orders  \
                           	 WHERE \
					Order_ID in (  \
                       	...
                       	SELECT \
				Order_ID, Payment_Method, Total_Price, Pincode, Street, Landmark, City, State, Type, Status  \
                        FROM \
				Address INNER JOIN T1  \
                        ON \
				Address.Address_ID = T1.Address_ID
        else:
            sql_stmt =  WITH T1 AS ( \
                            SELECT\
				 Order_ID, Address_ID, Payment_Method, Total_Price, Status  \
